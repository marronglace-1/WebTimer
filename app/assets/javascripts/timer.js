...

{
    ...
    function judgeOnTheListContent (newOption) {        //Contentに表示される値（Listモデルに保存される情報）を現状に合わせて登録するための処理。（Contentの削除後、トラブルにならないために必須。）
        const listsArray = selectList.children;         //現在Contentセレクタ(学習内容)に入っている内容をlistsArrayに代入。
        let listsValueArray = [];       
        for (let i = 0; i < listsArray.length; i++) {   //listsArrayのデータからValueを取得し配列に代入。
            listsValueArray[i] = parseInt(listsArray[i].value);
        };
        if (listsValueArray.includes(1)) {              //1～3の数字のうち、現状Valueに存在しない値をNewOptionのValueに代入する。
    ...
    function setContentLabels() {       //CONTENTの情報が保存されていない場合は、CONTENTの値を。未保存の場合はラベルに未設定という値をいれてグラフを表示させる処理。
        ...
        if (gon.list_content1) {
            ...
            if (listsValueArray.includes(1)) {
                contentLabel1 = listsArray[listsValueArray.indexOf(1)].textContent;
            } else {
                contentLabel1 = "未設定";    
            }
        ...
        if (gon.list_content2) {
            ...
            if (listsValueArray.includes(2)) {
                contentLabel2 = listsArray[listsValueArray.indexOf(2)].textContent;
            } else {
                contentLabel2 = "未設定";   
            }
        ...
        if (gon.list_content3) {
            ...
            if (listsValueArray.includes(3)) {
                contentLabel3 = listsArray[listsValueArray.indexOf(3)].textContent;
            } else {
                contentLabel3 = "未設定";
            }
